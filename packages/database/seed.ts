...
async function main() {
  ...
  const p1 = await prisma.product.create({ data: { shopId: shop.id, categoryId: cat1.id, name: 'Phone Pro Max', description: 'Latest phone', images: '[]', price: 5999, totalStock: 200, sales: 1580, status: 'active' } });
  await prisma.productSku.create({ data: { productId: p1.id, specs: '{\"color\":\"Silver\",\"storage\":\"256GB\"}', price: 5999, stock: 100 } });
  await prisma.productSku.create({ data: { productId: p1.id, specs: '{\"color\":\"Silver\",\"storage\":\"512GB\"}', price: 6999, stock: 60 } });
  await prisma.productSku.create({ data: { productId: p1.id, specs: '{\"color\":\"Black\",\"storage\":\"256GB\"}', price: 5999, stock: 40 } });
  const p2 = await prisma.product.create({ data: { shopId: shop.id, categoryId: cat1.id, name: 'Earphone Pro', description: 'Best earphone', images: '[]', price: 899, totalStock: 500, sales: 3200, status: 'active' } });
  await prisma.productSku.create({ data: { productId: p2.id, specs: '{\"color\":\"White\"}', price: 899, stock: 200 } });
  await prisma.productSku.create({ data: { productId: p2.id, specs: '{\"color\":\"Black\"}', price: 899, stock: 200 } });
  await prisma.productSku.create({ data: { productId: p2.id, specs: '{\"color\":\"Blue\"}', price: 949, stock: 100 } });
  const p3 = await prisma.product.create({ data: { shopId: shop.id, categoryId: cat2.id, name: 'Laptop 14inch', description: 'i7 laptop', images: '[]', price: 4999, totalStock: 100, sales: 890, status: 'active' } });
  await prisma.productSku.create({ data: { productId: p3.id, specs: '{\"color\":\"Gray\",\"ram\":\"16GB\"}', price: 4999, stock: 60 } });
  await prisma.productSku.create({ data: { productId: p3.id, specs: '{\"color\":\"Gray\",\"ram\":\"32GB\"}', price: 5999, stock: 40 } });
